"""Standalone Kaggle version of the Qwen3-Embedding-4B traceability run."""
...
def select_execution_device(requested_device: Optional[str]) -> str:
    """Choose CPU or GPU before loading Qwen."""
...
def load_model(device: Optional[str]) -> Any:
    """Load the pinned Qwen SentenceTransformer model."""
...
def encode_texts(
    ...
    """Encode texts with Qwen and return float32 NumPy embeddings."""
    if not texts:
...
def run_pipeline(
    ...
    """Run the complete standalone Qwen pipeline and write JSON outputs."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data", default="/kaggle/input/datasets/omeralicelikci/kaggle-data/Kaggle Data2/Filtered models/filtered_elements_gemma3_focus.json")
    parser.add_argument("--output", default="/kaggle/working/qwen3_embedding_4b_matches_Focus_Gemma3TEST3.json")
...
def main() -> None:
    """Run the standalone Qwen script and print a short summary."""
    args = parse_args()
    stats = run_pipeline(
